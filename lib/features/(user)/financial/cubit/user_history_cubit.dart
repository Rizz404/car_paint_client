...
class UserHistoryCubit extends Cubit<BaseState> with Cancelable {
  final UserHistoryRepo historyRepo;
  UserHistoryCubit({
  ...
  Future<void> getHistory(int page, CancelToken cancelToken,
      {int limit = 10}) async {
    cancelRequests();
    ...
    try {
      await handleBaseCubit<PaginatedData<Transactions>>(
        emit,
        () => historyRepo.getHistory(page, limit, cancelToken),
  ...
  Future<void> loadNextPage(
    CancelToken cancelToken,
      ...
      getHistory(currentPage + 1, cancelToken);
  Future<void> refresh(
    int limit,
    CancelToken cancelToken,
  ) async {
    ...
    isLoadingMore = false;
    emit(const BaseLoadingState());
    await getHistory(currentPage, cancelToken, limit: limit);
  }
}
